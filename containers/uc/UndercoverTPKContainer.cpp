...
namespace EAGLEye
{
    ...
    {
        UndercoverTPKContainer::UndercoverTPKContainer(std::ifstream &stream, uint32_t containerSize)
                : Container(stream),
                  m_containerSize(containerSize)
        {
        }

        std::shared_ptr<TPK> UndercoverTPKContainer::Get()
        {
        ...
        size_t UndercoverTPKContainer::ReadChunks(uint32_t totalSize)
        {
            ...
            for (int i = 0; i < 0xFFFF && m_stream.tellg() < runTo; i++)
            {
                ...
                {
                    case UNDERCOVER_TPK:
                    case UNDERCOVER_TPK_TEXTURE_DATA_ROOT:
                    {
                        bytesRead += this->ReadChunks(size);
                        break;
                    }
                    case UNDERCOVER_TPK_PART_INFO:
                    {
                        m_stream.ignore(4);
                        bytesRead += 4;
                    ...
                    case UNDERCOVER_TPK_PART_HASHES:
                    {
                    ...
                    case UNDERCOVER_TPK_PART_HEADERS:
                    {
                    ...
                    case UNDERCOVER_TPK_PART_DXT_HEADERS:
                    {
                    ...
                    // Texture Data
//                    case UNDERCOVER_TPK_TEXTURE_DATA_CONTAINER:
//                    {
