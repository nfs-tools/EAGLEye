...
namespace EAGLEye
{
    namespace Carbon
    {
        CarbonBINChunk g_chunks[] = {
                ...
                {ID_TEX_BINARYDATA,  0x80, "Texture Data"},
                {ID_ALIGN,           0x00, nullptr},
        };

        std::ofstream currentModelDumpStream;
        std::shared_ptr<boost::filesystem::path> currentDumpPath = nullptr;
            ...
            for (int i = 0; i < 0xFFFF && ifstream.tellg() < runTo; i++)
            {
                ...
                switch (partId)
                {
                    case BCHUNK_CARBON_SOLIDS_ARCHIVE:
                    {
                        ...
                        boost::filesystem::path sectionDir = boost::filesystem::relative("data/carbon/dumps/");
                        sectionDir.append(geometryFileInfo.section);

                        if (!boost::filesystem::is_directory(sectionDir))
                            boost::filesystem::create_directories(sectionDir);

                        boost::filesystem::path archiveDir = sectionDir;
                        archiveDir.append("/");
                        archiveDir += boost::filesystem::path(std::string(geometryFileInfo.path)).stem();

                        if (!boost::filesystem::is_directory(archiveDir))
                            boost::filesystem::create_directories(archiveDir);

                        currentDumpPath = std::make_unique<boost::filesystem::path>(archiveDir);
                    ...
                    {
                        auto beginPos = (long) ifstream.tellg();
                        ...
                        ifstream.seekg(beginPos);

                        auto filePath = std::make_shared<boost::filesystem::path>(*currentDumpPath)->append("/").append(
                                std::string(name).append(".geodump")).string();
                        printf("%s\n", filePath.c_str());

                        currentModelDumpStream = std::ofstream(filePath, std::ios::trunc);
                        currentModelDumpStream << "Generated by EAGLEye - debug dump" << std::endl;
                        currentModelDumpStream << std::endl;
                        currentModelDumpStream << "############### Object Header ###############" << std::endl;
                        currentModelDumpStream << "Name:          " << name << std::endl;
                        currentModelDumpStream << "Minimum Point: " << ptMin.x << "/" << ptMin.y << "/" << ptMin.z
                                               << "/"
                                               << ptMin.w << std::endl;
                        currentModelDumpStream << "Maximum Point: " << ptMax.x << "/" << ptMax.y << "/" << ptMax.z
                                               << "/"
                                               << ptMax.w << std::endl;
                        currentModelDumpStream << std::endl;
                        currentModelDumpStream << "# Hex Dump" << std::endl;
                        dumpBytesToFile(ifstream, currentModelDumpStream, partSize);
                    ...
                    case BCHUNK_CARBON_MESH_DESCRIPTOR:
                    {
                        ...
                        currentModelDumpStream << "###############   Mesh Info   ###############" << std::endl;
                        currentModelDumpStream << std::endl;
                        currentModelDumpStream << "# Header Dump" << std::endl;
                    ...
                    case BCHUNK_CARBON_MATERIAL_NAME:
                    {
                        ...
                        currentModelDumpStream << "# Material: " << name << std::endl;
                    ...
                    case BCHUNK_CARBON_MESH_FACES:
                    {
                        ...
                        currentModelDumpStream << "# Faces Dump (# of Faces - " << numFaces << "): " << std::endl;
                    ...
                    case BCHUNK_CARBON_MESH_VERTICES:
                    {
                        ...
                        currentModelDumpStream << "# Vertices Dump (# of Vertices - " << numVertices << "): "
                                               << std::endl;
...
//                        dumpBytes(ifstream, partSize - partBytesRead);
                    ...
                    case BCHUNK_CARBON_TPK_TEXTURE_NAMES: // TPK texture names
                    {
                        ...
                        for (int j = 0; j < numTextures; j++)
                        {
                            int32_t hash, type;
                            partBytesRead += readGeneric(ifstream, hash);
                            partBytesRead += readGeneric(ifstream, type);

                            uint32_t dataOffset;
                            partBytesRead += readGeneric(ifstream, dataOffset);

                            ifstream.ignore(4);
                            partBytesRead += 4;

                            uint32_t dataSize;
                            partBytesRead += readGeneric(ifstream, dataSize);

                            ifstream.ignore(8);
                            partBytesRead += 8;

                            uint32_t resolution;
                            partBytesRead += readGeneric(ifstream, resolution);

                            uint32_t mipMap;
                            partBytesRead += readGeneric(ifstream, mipMap);

                            ifstream.ignore(40);
                            partBytesRead += 40;

                            BYTE nameLength;
                            partBytesRead += readGeneric(ifstream, nameLength);

                            char name[nameLength];
                            partBytesRead += readGenericArray(ifstream, name, nameLength);
...
//                        dumpBytes(ifstream, partSize - partBytesRead);
                    ...
                    case BCHUNK_CARBON_TPK_TEXTURE_HEADERS:
                    {
                        for (int j = 0; j < numTextures; j++)
                        {
                            BYTE header[0x18];
                            partBytesRead += readGenericArray(ifstream, header, 0x18);

                            hexdump(stdout, &header);
                        }

                        break;
                    }
                    case BCHUNK_CARBON_TPK_FULL_DATA_BLOCK:
                    {
                        break;
                    }
                    default:
                ...
                if (partBytesRead > partSize)
                {
                    fprintf(stderr, "STOP! Overran buffer - tried to read %zu bytes when we only have %d available\n",
                            partBytesRead, partSize);
                    exit(1);
                }
        ...
        void ParseTPKChunk(std::ifstream &ifstream, uint32_t id, uint32_t size)
        {
            ...
            for (int i = 0; i < 0xFFFF && ifstream.tellg() < runTo; i++)
            {
                ...
                switch (scId)
                {
                    case BCHUNK_CARBON_TPK_CONTAINER:
                    case BCHUNK_CARBON_TPK_TEXTURE_DATA:
                    {
...
//            dumpBytes(ifstream, size - bytesRead);
        ...
        void ParseCarGeometryChunk(std::ifstream &ifstream, uint32_t id, uint32_t size)
        {
            ...
            while (ifstream.tellg() < runTo)
            {
                ...
                bytesRead += readGenericArray(ifstream, tmp, 4);

                if (tmp[0] == 'J' && tmp[1] == 'D' && tmp[2] == 'L' && tmp[3] == 'Z')
                {
                    printf("'JDLZ' at %ld / %ld\n", (long) ifstream.tellg(), runTo);
                }
            }

//            printf("at %ld - runTo = %ld\n", (long) ifstream.tellg(), runTo);
//
//            dumpBytes(ifstream, boost::algorithm::clamp(size - bytesRead, 0, 512));
//            ifstream.ignore(size - bytesRead);
        }
